...
// WithCoin: total coins staked (non-spendable until maturity)
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
		...
		//WithCoin now iterates each utxo inside of CheckStakeKernelHash()
...
// WithCoin: check 'spent' consistency between wallet and txindex
// WithCoin: fix wallet spent state according to txindex
...
// WithCoin: disable transaction (only for coinstake)
