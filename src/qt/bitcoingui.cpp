    ...
    rpcConsole(0)
{
    setMinimumSize(870,620);
            resize(870,620);
    setWindowTitle(tr("WithCoin") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a WithCoin address"));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About WithCoin"), this);
    aboutAction->setToolTip(tr("Show information about WithCoin"));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for WithCoin"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("WithCoin client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("WithCoin client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to WithCoin network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid WithCoin address or malformed URI parameters."));
    }
