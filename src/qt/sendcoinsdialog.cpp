    ...
    model(0)
{
	...
	ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a WithCoin address (e.g. CfwYhBmGXcFP2Po1NpRUEiK8km2)"));
...
void SendCoinsDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        ...
        else if (!CBitcoinAddress(text.toStdString()).IsValid())
        {
            ...
            ui->labelCoinControlChangeLabel->style()->polish(ui->labelCoinControlChangeLabel);
            ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid WithCoin address"));
        }
        else
        {
